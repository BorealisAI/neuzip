...
def parse_args():
    ...
    parser.add_argument("--load_in_low_bit", action="store_true")
...
def main():
    ...
    if args.memory_efficient == "qlora":
        ...
        peft_config = peft.LoraConfig(
            ...
            # modules_to_save=["wte", "lm_head", "embed", "embed_tokens"],
            task_type="CAUSAL_LM",
        )
    ...
    eval_words = len("\n".join(raw_datasets["validation"][text_column_name]).split())
    ...
    eval_dataloader = DataLoader(
        eval_dataset,
        collate_fn=default_data_collator,
        batch_size=args.per_device_eval_batch_size or args.per_device_train_batch_size,
    )
    ...
    accelerator.init_trackers(
        ...
        init_kwargs={
            "wandb": {
                "name": f"{args.memory_efficient}-lr{args.learning_rate}{'-lomo' if args.lomo else ''}-seed{args.seed}"
            }
        },
    )
    ...
    def evaluate(model):
        ...
        try:
            eval_loss = torch.sum(losses / eval_words).item()
            ...
            with accelerator.accumulate(model):
                ...
                if args.memory_efficient == "neuzip":
                    ...
                    for p in model.parameters():
                        if isinstance(p, neuzip.nn.Parameter):
                            if getattr(p, "_gradient", None) is not None:
                                p._manager.write(
                                    p._manager.read(p._handle).add_(
                                        p._gradient, alpha=-lr
                                    ),
                                    p._handle,
                                )
                                p._gradient = None
    ...
    accelerator.log(
        {
            ...
            "average_time_per_batch": training_time / completed_steps,
        },
        step=completed_steps,
    )
